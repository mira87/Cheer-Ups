...
router.post("/find", (req, res) => {
  CheerUpModel.findOne({ title: req.body.title }).then(myInstance => {
...
router.get("/edit/:title", (req, res) => {
  CheerUpModel.findOne({ title: req.params.title }).then(myInstance =>
    res.render("edit", { myInstance })
  );
});

router.put("/edit/:title", (req, res) => {
  CheerUpModel.findOneAndUpdate({ title: req.params.title }, req.body, {
    new: true
  }).then(cheer => {
    res.redirect("/");
  });
